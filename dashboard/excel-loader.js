/* dashboard/excel-loader.js
   Pure JS file (NO <script> tags inside!)
   Loads Dashboard.xlsx via fetch + xlsx, maps to window.IMMO_DATA
*/
...
(function () {
  ...
  async function ensureXlsx() {
    ...
    await new Promise((resolve, reject) => {
      ...
      s.onload = resolve;
      s.onerror = () => reject(new Error("XLSX CDN konnte nicht geladen werden"));
      document.head.appendChild(s);
    });
    if (!window.XLSX) throw new Error("XLSX nicht verfügbar nach CDN-Load");
  }

  async function load(path) {
    // Always set fallback first (prevents empty modules)
    ...
    try {
      ...
      if (!res.ok) throw new Error("Excel fetch failed: HTTP " + res.status + " (" + path + ")");
