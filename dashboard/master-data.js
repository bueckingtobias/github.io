...
(function () {
  ...
  // KPI series (replace with real values anytime)
  ...
  const projectsGesamt = {
    ...
    Notizen:
      "Gesamtübersicht über alle Gewerke. Werte werden aus Projects_Gewerke aggregiert (im Modul).",
  };

  // 10 Gewerke/Handwerker (Beispieldaten)
  const projectsGewerkeRaw = [
  ...
  const projectsGewerke = projectsGewerkeRaw.map((r) => {
    const paid = Number(r.Gezahlt) || 0;
    const offer = Number(r.Angebot) || 0;
    const prog = Number(r.Baufortschritt) || 0;
  ...
  const finance = {
    ...
    op: [
      {
        Titel: "Offene Posten (OP)",
        Betrag: 8400,
        Faellig_am: new Date(Y, M, 10).toISOString().slice(0, 10),
        Status: "offen",
        Kommentar: "Beispiel: Handwerkerrechnung",
      },
      {
        Titel: "Mietrückstand",
        Betrag: 1200,
        Faellig_am: new Date(Y, M, 5).toISOString().slice(0, 10),
        Status: "in Klärung",
        Kommentar: "Beispiel: Mieter X",
      },
    ],
